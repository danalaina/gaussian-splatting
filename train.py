...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            # Progress bar
            ema_loss_for_log = 0.4 * loss.item() + 0.6 * ema_loss_for_log  # Exponential moving average
            if iteration % 10 == 0:
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians, *renderArgs, scene.tensorVMsplit)["render"], 0.0, 1.0)
        ...
        if tb_writer:
            tb_writer.add_histogram("scene/opacity_histogram", scene.tensorVMsplit.compute_densityfeature(scene.tensorVMsplit.normalize_coord(scene.gaussians._xyz)).unsqueeze(1), iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[2_000, 5_000, 10_000, 20_000, 30_000]) # 7k
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[2_000, 5_000, 10_000, 20_000, 30_000])
