...
class TensorVMSplit(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        ...
        self.basis_mat = torch.nn.Sequential(
                                            torch.nn.Linear(sum(self.app_n_comp), self.app_dim, bias=False),
                                            torch.nn.ReLU()
                                            ).to(device)
        self.rot_plane, self.rot_line = self.init_one_svd(self.app_n_comp, self.gridSize, 0.1, device)
        self.rot_basis_mat = torch.nn.Sequential(
    ...
    def init_one_svd(self, n_component, gridSize, scale, device):
        ...
        for i in range(len(self.vecMode)):
            ...
            plane_coef.append(torch.nn.Parameter(
                scale * torch.rand((1, n_component[i], gridSize[mat_id_1], gridSize[mat_id_0]))))  #
            line_coef.append(
                torch.nn.Parameter(scale * torch.rand((1, n_component[i], gridSize[vec_id], 1))))
    ...
    def get_rotation(self, pre_rotation):
        return torch.nn.functional.normalize(pre_rotation)
    
    def compute_rotfeature(self, xyz_sampled):
        ...
        plane_coef_point,line_coef_point = [],[]
        for idx_plane in range(len(self.rot_plane)):
            plane_coef_point.append(F.grid_sample(self.rot_plane[idx_plane], coordinate_plane[[idx_plane]],
                                                align_corners=True).view(-1, *xyz_sampled.shape[:1]))
            line_coef_point.append(F.grid_sample(self.rot_line[idx_plane], coordinate_line[[idx_plane]],
                                            align_corners=True).view(-1, *xyz_sampled.shape[:1]))
        plane_coef_point, line_coef_point = torch.cat(plane_coef_point), torch.cat(line_coef_point)
    ...
    def shrink(self, new_aabb):
        ...
        t_l = t_l.clamp(min=0)  # prevent the situation of expansion instead of shrinking
